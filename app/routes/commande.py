...
def commande():
    ...
    inscr_valide = []
    inscr_nom_coursier = []
    inscriptions = participants_amp.Participants_amp.query.filter_by(id_user=current_user.id).join(amplet.Amplets,participants_amp.Participants_amp.id_amp==amplet.Amplets.id).filter_by(navette=0).all()
    # print(inscriptions)
    if inscriptions:
